...
Как мы видим, обучение использованию **IDA** позволит нам расширить наше рабочее пространство. Хотя в этой серии туториалов мы будем сосредотачиваться на **32**-х и **64**-х битной **WINDOWS** в пользовательском режиме, но иногда мы будем работать и в режиме ядра. Сам факт знакомства с инструментом позволит нам легко адаптироваться к любому её использованию.

Идея этих руководств состоит в том, чтобы начать с нуля. Другими словами, многие вещи, которые мы видели во "**ВВЕДЕНИИ В ОТЛАДКУ С НУЛЯ С ИСПОЛЬЗОВАНИЕМ OLLYDBG"**, будут видны здесь снова, но в **IDA** я постараюсь объяснить больше.

Если кто-то видит, что то, что он читает является слишком сложным, то сначала будет правильным прочитать серию туториалов "**ВВЕДЕНИЕ В ОТЛАДКУ С НУЛЯ С ИСПОЛЬЗОВАНИЕМ OLLYDBG"**, которые немного проще.В этом курсе будет все: статический и динамический реверсинг, крэкинг, эксплоитинг, распаковка. Мы будем стараться охватить как можно больше тем, начиная с нуля.
...
После установки мы можем запустить **IDA** впервые. И, как всегда, мы будем открывать, как и в любом другом нашем курсе, которым мы гордимся, крэкми **CRUEHEAD**, который будет приложен к этому туториалу.
...
В то время как крэкми **CRUEHEAD**, который является **32**-х битным после **PE** имеет такие символы.
...
В **OPEN SUBVIEW** в **ЗАГРУЗЧИКЕ** у нас есть вкладка с шестнадцатеричным представлением или **HEX DUMP**.

![](.gitbook/assets/01/18.png)

В **OPEN SUBVIEW** мы можем увидеть вкладку импортированных функций или **IMPORTS**.

![](.gitbook/assets/01/19.png)

Также в **VIEW** активирован пункт **GRAPH OVERVIEW**, который представляет собой навигатор по графике видимой функции. Мы можем перемещать и изменять часть, которая в данный момент отображается на экране.
...
Конечно, в нашем первом туториале мы сделали быстрый обзор программы. В следующих туториалах мы будем двигаться быстрее.
